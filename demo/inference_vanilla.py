...
class LiveInfer:
    def __init__(self, max_frames_on_gpu=50) -> None:
        args = parse_args()
        ...
        self.v_placeholder_id = self.model.config.v_placeholder_id
        ...
        # GPU memory management for video frames (ring buffer)
        self.max_frames_on_gpu = max_frames_on_gpu  # Maximum number of raw frames to keep on GPU
        self.gpu_frame_buffer = []  # Ring buffer for GPU frames
        self.buffer_start_idx = 0  # Starting frame index in the buffer
    ...
    def _call_for_response(self, video_time, query):
        ...
        output_ids, self.past_key_values = fast_greedy_generate(model=self.model, input_ids=self.last_ids, inputs_embeds=inputs_embeds, past_key_values=self.past_key_values, eos_token_id=self.eos_token_id, inplace_output_ids=self.inplace_output_ids)
        self.last_ids = self.eos_token_id # force end when max length reached
        if query:
    ...
    def _call_for_streaming(self, ):
        while self.frame_embeds_queue:
            ...
            if not self.past_key_values:
                ...
                self.last_ids = torch.cat([torch.tensor([[self.eos_token_id]], device='cuda'), self._added_stream_prompt_ids], dim=1)
            input_ids = torch.cat([
                self.last_ids,
                torch.tensor([[self.v_placeholder_id] * self.frame_num_tokens], device='cuda')
            ], dim=1)
    ...
    def _get_frames_on_gpu(self, frame_indices):
        """
        Get frames on GPU using a simple ring buffer.
        Since we process frames sequentially, we just need to keep a sliding window.
        """
        frames_on_gpu = []
        
        for frame_idx in frame_indices:
            # Calculate position in ring buffer
            buffer_idx = frame_idx - self.buffer_start_idx
            
            # Check if frame is still in buffer
            if 0 <= buffer_idx < len(self.gpu_frame_buffer):
                frames_on_gpu.append(self.gpu_frame_buffer[buffer_idx])
            else:
                # Frame not in buffer, need to load from CPU
                # If buffer is full, remove old frames from the front
                if len(self.gpu_frame_buffer) >= self.max_frames_on_gpu:
                    # Remove frames that are too old
                    frames_to_remove = frame_idx - self.buffer_start_idx - self.max_frames_on_gpu + 1
                    if frames_to_remove > 0:
                        self.gpu_frame_buffer = self.gpu_frame_buffer[frames_to_remove:]
                        self.buffer_start_idx += frames_to_remove
                
                # Load frame to GPU
                frame_gpu = self.video_tensor[frame_idx].to('cuda', non_blocking=True)
                self.gpu_frame_buffer.append(frame_gpu)
                frames_on_gpu.append(frame_gpu)
        
        return frames_on_gpu
    ...
    def input_video_stream(self, video_time):
        ...
        if frame_idx > self.last_frame_idx:
            ...
            # Load frames on-demand from CPU to GPU and encode them
            frames_on_gpu = self._get_frames_on_gpu(list(ranger))
            frames_stacked = torch.stack(frames_on_gpu, dim=0)
            frames_embeds = self.model.visual_embed(frames_stacked).split(self.frame_num_tokens)
    ...
    def load_video(self, video_path):
        # Keep video tensor on CPU to save GPU memory
        ...
        logger.warning(f'{video_path} -> {self.video_tensor.shape}, {self.frame_fps} FPS (keeping on CPU, will transfer to GPU on-demand)')

    def load_video_tensor(self, tensor_path):
        self.frame_embeds = torch.load(tensor_path).to('cuda')
        self.num_video_frames = self.frame_embeds.size(0)
        self.video_duration = self.frame_embeds.size(0) / self.frame_fps
        logger.warning(f'{tensor_path} -> {self.frame_embeds.shape}, {self.frame_fps} FPS')
